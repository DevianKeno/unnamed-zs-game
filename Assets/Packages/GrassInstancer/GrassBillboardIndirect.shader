    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            VertexOutput vert(VertexInput v, uint instanceID : SV_InstanceID)
            {
                ...
                float3 positionWorldSpace = mul(mat, float4(v.vertex.xyz, 1)).xyz;
                o.staticWorldPos = positionWorldSpace;

                //move world UVs by time
                float4 worldPos = float4(positionWorldSpace, 1);
                float2 worldUV = (worldPos.xz + _WindSpeed.xy * _Time.y); 
                ...
                //apply distortion
                positionWorldSpace.xz += distortion * _WindStrength * normalize(_WindSpeed.xy);
